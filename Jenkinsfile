    ...
    stages {
        ...
        stage('lint') {
            steps {
                ...
                sh 'docker run --rm -i -v $(pwd):/mnt verilator --lint-only -Wall go-teak/src/smi/verilog/*.v go-teak/src/sdaccel/stubs/*.v go-teak/src/sdaccel/verilog/*.v --top-module sda_kernel_wrapper_gmem --report-unoptflat'
            }
        }
